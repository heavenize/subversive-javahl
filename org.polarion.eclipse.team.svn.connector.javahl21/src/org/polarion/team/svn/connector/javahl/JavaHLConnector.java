 ...
 */
public class JavaHLConnector extends JavaHLService implements ISVNConnector {
	// Constants removed from Subversive 5.1.0 API - defined locally for backward compatibility
	private static final String IS_CREDENTIALS_CACHE_ENABLED = "isCredentialsCacheEnabled";
	private static final String SET_CREDENTIALS_CACHE_ENABLED = "setCredentialsCacheEnabled";
	private static final String SET_PROXY = "setProxy";
	private static final String SET_CLIENT_SSL_CERTIFICATE = "setClientSSLCertificate";
	private static final String IS_SSL_CERTIFICATE_CACHE_ENABLED = "isSSLCertificateCacheEnabled";
	private static final String SET_SSL_CERTIFICATE_CACHE_ENABLED = "setSSLCertificateCacheEnabled";
	private static final String SET_SSH_CREDENTIALS_PASSWORD = "setSSHCredentialsPassword";
	private static final String SET_COMMIT_MISSING_FILES = "setCommitMissingFiles";
	private static final String IS_COMMIT_MISSING_FILES = "isCommitMissingFiles";
...
//		}
	...
	public boolean isCredentialsCacheEnabled() {
		...
		this.fireAsked(IS_CREDENTIALS_CACHE_ENABLED, null);
		this.fireSucceeded(IS_CREDENTIALS_CACHE_ENABLED, null, Boolean.TRUE);
		return true;
	}
	...
	public void setCredentialsCacheEnabled(boolean cacheCredentials) {
		...
		this.fireAsked(SET_CREDENTIALS_CACHE_ENABLED, parameters);
		cacheCredentials = ((Boolean)parameters.get("cacheCredentials")).booleanValue();
		this.fireFailed(SET_CREDENTIALS_CACHE_ENABLED, parameters, null);
	}
	...
	public void setProxy(String host, int port, String username, String password) {
		...
		parameters.put("username", username);
		parameters.put("password", password);
		this.fireAsked(SET_PROXY, parameters);
		host = (String)parameters.get("host");
		port = ((Integer)parameters.get("port")).intValue();
		username = (String)parameters.get("username");
		password = (String)parameters.get("password");
		
		this.fireFailed(SET_PROXY, parameters, null);
	}
	...
	public void setClientSSLCertificate(String certPath, String passphrase) {
		...
		this.fireAsked(SET_CLIENT_SSL_CERTIFICATE, parameters);
		...
		this.fireSucceeded(SET_CLIENT_SSL_CERTIFICATE, parameters, null);
	}

	public boolean isSSLCertificateCacheEnabled() {
		// SSL certificate cache is always enabled for native connector
		this.fireAsked(IS_SSL_CERTIFICATE_CACHE_ENABLED, null);
		this.fireSucceeded(IS_SSL_CERTIFICATE_CACHE_ENABLED, null, Boolean.TRUE);
		return true;
	}
	...
	public void setSSLCertificateCacheEnabled(boolean enabled) {
		...
		this.fireAsked(SET_SSL_CERTIFICATE_CACHE_ENABLED, parameters);
		enabled = ((Boolean)parameters.get("enabled")).booleanValue();
		
		this.fireFailed(SET_SSL_CERTIFICATE_CACHE_ENABLED, parameters, null);
	}
	...
	public void setSSHCredentials(String username, String privateKeyPath, String passphrase, int port) {
		...
		this.fireFailed(ISVNCallListener.SET_SSH_CREDENTIALS, parameters, null);
	}
	...
	public void setSSHCredentials(String username, String password, int port) {
		...
		parameters.put("password", password);
		parameters.put("port", Integer.valueOf(port));
		this.fireAsked(SET_SSH_CREDENTIALS_PASSWORD, parameters);
		username = (String)parameters.get("username");
		password = (String)parameters.get("password");
		port = ((Integer)parameters.get("port")).intValue();
		
		this.fireFailed(SET_SSH_CREDENTIALS_PASSWORD, parameters, null);
	}
	...
	public void setCommitMissingFiles(boolean commitMissingFiles) {
		...
		this.fireAsked(SET_COMMIT_MISSING_FILES, parameters);
		commitMissingFiles = ((Boolean)parameters.get("commitMissingFiles")).booleanValue();
		
		this.commitMissingFiles = commitMissingFiles;
		
		this.fireSucceeded(SET_COMMIT_MISSING_FILES, parameters, null);
	}

	public boolean isCommitMissingFiles() {
		this.fireAsked(IS_COMMIT_MISSING_FILES, null);
		this.fireSucceeded(IS_COMMIT_MISSING_FILES, null, Boolean.valueOf(this.commitMissingFiles));
	...
	public void add(String path, SVNDepth depth, long options, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.add(path, ConversionUtility.convertDepth(depth), (options & Options.FORCE) != 0, (options & Options.INCLUDE_IGNORED) != 0, false, (options & Options.INCLUDE_PARENTS) != 0);
		...
		try {
			if (this.commitMissingFiles) {
				for (int i = 0; i < path.length && !monitor.isActivityCancelled(); i++) {
					this.client.status(path[i], ConversionUtility.convertDepth(depth), false, true, false, false, true, false, null, new StatusCallback() {
						public void doStatus(String path, Status status) {
							if (status.getTextStatus() == org.apache.subversion.javahl.types.Status.Kind.missing) {
								try {
									JavaHLConnector.this.client.remove(new HashSet<String>(Arrays.asList(new String[] {path})), true, false, null, null, null);
								} 
								catch (ClientException e) {
								}
							}
							else if (status.getTextStatus() == org.apache.subversion.javahl.types.Status.Kind.unversioned) {
								try {
									JavaHLConnector.this.client.add(path, org.apache.subversion.javahl.types.Depth.empty, false, false, false, false);
								} 
								catch (ClientException e) {
								}
							}
			...
			if (depth != SVNDepth.INFINITY) {
				...
				for (int i = 0; i < path.length && !monitor.isActivityCancelled(); i++) {
					...
					if (toCheck.isDirectory()) {
						final org.apache.subversion.javahl.types.Status []st = new org.apache.subversion.javahl.types.Status[1];
						this.client.status(path[i], ConversionUtility.convertDepth(SVNDepth.EMPTY), false, true, false, false, false, false, null, new org.apache.subversion.javahl.callback.StatusCallback() {
							public void doStatus(String path, org.apache.subversion.javahl.types.Status status) {
								st[0] = status;
							}
						});
						if (st[0] != null && st[0].getTextStatus() == org.apache.subversion.javahl.types.Status.Kind.deleted) {
							deleted.add(path[i]);
							String root = path[i];
	...
	public void revert(String []paths, SVNDepth depth, String []changeLists, long options, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			...
			this.client.revert(pathSet, ConversionUtility.convertDepth(depth), changeLists == null ? null : Arrays.asList(changeLists), false, false);
	...
	public void status(String path, SVNDepth depth, long options, String[] changeLists, ISVNEntryStatusCallback callback, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.status(path, ConversionUtility.convertDepth(depth), (options & Options.SERVER_SIDE) != 0, true, (options & Options.INCLUDE_UNCHANGED) != 0, (options & Options.INCLUDE_IGNORED) != 0, (options & Options.IGNORE_EXTERNALS) != 0, false, changeLists == null ? null : Arrays.asList(changeLists), ConversionUtility.convert(callback));
	...
	public void merge(SVNEntryReference reference, SVNRevisionRange []revisions, String localPath, SVNDepth depth, long options, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.merge(reference.path, ConversionUtility.convert(reference.pegRevision), Arrays.asList(ConversionUtility.convert(revisions)), localPath, (options & Options.FORCE) != 0, ConversionUtility.convertDepth(depth), (options & Options.IGNORE_ANCESTRY) != 0, (options & Options.SIMULATE) != 0, (options & Options.RECORD_ONLY) != 0);
	...
	public void importTo(String path, String url, String message, SVNDepth depth, long options, Map revProps, ISVNImportFilterCallback filter, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.doImport(path, url, ConversionUtility.convertDepth(depth), (options & Options.INCLUDE_IGNORED) != 0, (options & Options.IGNORE_UNKNOWN_NODE_TYPES) != 0, ConversionUtility.convertRevPropsToSVN(revProps), new CommitMessage(message), new CommitInfo(monitor));
	...
	public void diffTwo(SVNEntryRevisionReference reference1, SVNEntryRevisionReference reference2, String relativeToDir, OutputStream stream, SVNDepth depth, long options, String[] changeLists, long outputOptions, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			...
			this.handleCustomException(ex, ISVNCallListener.DIFF_TWO_STREAM, parameters);
		}
		finally {
			if (inputStream != null) {
	...
	public void moveLocal(String[] srcPaths, String dstPath, long options, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.move(new HashSet<String>(Arrays.asList(srcPaths)), dstPath, (options & Options.FORCE) != 0, (options & Options.INTERPRET_AS_CHILD) != 0, false, null, null, null);
	...
	public void moveRemote(String[] srcPaths, String dstPath, String message, long options, Map revProps, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			this.client.move(new HashSet<String>(Arrays.asList(srcPaths)), dstPath, (options & Options.FORCE) != 0, (options & Options.INTERPRET_AS_CHILD) != 0, (options & Options.INCLUDE_PARENTS) != 0, ConversionUtility.convertRevPropsToSVN(revProps), new CommitMessage(message), new CommitInfo(monitor));
	...
	public void copyLocal(SVNEntryRevisionReference []srcPaths, String destPath, long options, Map<String, List<SVNExternalReference>> externalsToPin, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			this.composite.add(wrapper);
			wrapper.start();
			org.apache.subversion.javahl.types.CopySource []srcs = new org.apache.subversion.javahl.types.CopySource[srcPaths.length];
			for (int i = 0; i < srcPaths.length; i++) {
				// hack for JavaHL 1.6.x bug: JavaHL 'copy' method tries to find .entries file on the repository when WORKING revision is specified 
				srcs[i] = new org.apache.subversion.javahl.types.CopySource(srcPaths[i].path, srcPaths[i].revision != null && srcPaths[i].revision.getKind() == Kind.WORKING ? null : ConversionUtility.convert(srcPaths[i].revision), null);
			}
			this.client.copy(Arrays.asList(srcs), destPath, true, false, (options & Options.IGNORE_EXTERNALS) != 0, null, null, null);
...
//		}
	...
	protected class RepositoryInfoPrompt implements org.apache.subversion.javahl.callback.UserPasswordCallback {
		protected ISVNCredentialsPrompt prompt;
