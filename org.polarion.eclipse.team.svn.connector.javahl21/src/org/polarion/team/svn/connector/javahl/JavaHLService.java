...
public abstract class JavaHLService {
	...
	private synchronized static ProgressMonitorThread getProgressMonitorThread() {
		if (JavaHLService.monitorWrapperThread == null) {
			JavaHLService.monitorWrapperThread = new ProgressMonitorThread();
			JavaHLService.monitorWrapperThread.start();
		}
		return JavaHLService.monitorWrapperThread;
	}
	...
	protected class ProgressMonitorWrapper implements ISVNNotificationCallback, ISVNRepositoryNotificationCallback {
		...
		public void notify(SVNNotification info) {
			this.monitor.progress(this.current++, ISVNProgressMonitor.TOTAL_UNKNOWN, JavaHLService.makeItemState(info));
		}
