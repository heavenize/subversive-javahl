...
public class JavaHLManager extends JavaHLService implements ISVNManager {
	...
	public void load(String path, InputStream dataInput, SVNRevisionRange range, String relativePath, ISVNRepositoryNotificationCallback callback, long options, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		try {
			...
			this.svnAdmin.load(new File(path), dataInput, (options & Options.IGNORE_UUID) != 0, (options & Options.FORCE_UUID) != 0, (options & Options.USE_PRECOMMIT_HOOK) != 0, (options & Options.USE_POSTCOMMIT_HOOK) != 0, relativePath, ConversionUtility.convert(this.composite));
	...
	public void freeze(ISVNRepositoryFreezeAction action, String[] paths, ISVNProgressMonitor monitor) throws SVNConnectorException {
		...
		this.fireFailed(ISVNCallListener.FREEZE, parameters, null);
	}
